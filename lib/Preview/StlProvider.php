...
class StlProvider implements IProviderV2 {
    ...
    public function getThumbnail(File $file, int $maxX, int $maxY): ?IImage {
        ...
        try {
            ...
            // Downsample triangles for rapid preview rendering (target ~2000 triangles)
            $step = max(1, (int)($numTri / 2000));
            $triangles = [];
            ...
            for ($i = 0; $i < $numTri; $i++) {
                ...
                // Soft studio diffuse lighting
                $dot = max(0.35, min(1.0, 0.4 + ($nx * 0.4 + $ny * 0.4 + $nz * 0.7)));
            ...
            foreach ($triangles as $tri) {
                ...
                $px1 = ($size / 2) + ($tx1 - $ty1) * $cos30 * $scale;
                $py1 = ($size / 2) - (($tx1 + $ty1) * $sin30 + $tz1) * $scale;
                $px2 = ($size / 2) + ($tx2 - $ty2) * $cos30 * $scale;
                $py2 = ($size / 2) - (($tx2 + $ty2) * $sin30 + $tz2) * $scale;
                $px3 = ($size / 2) + ($tx3 - $ty3) * $cos30 * $scale;
                $py3 = ($size / 2) - (($tx3 + $ty3) * $sin30 + $tz3) * $scale;

                $dot = $tri[9];
                // Clean Sky-blue CAD theme
                $r = (int)(56 * $dot); $g = (int)(189 * $dot); $b = (int)(248 * $dot);
